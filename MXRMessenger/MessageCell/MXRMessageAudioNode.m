...
-(void)createDurationTextField {
    if (_durationTextNode == nil) {
        ...
        _durationTextNode.attributedText = @"00:00";
...
-(ASLayoutSpec *)layoutSpecThatFits:(ASSizeRange)constrainedSize {
    ...
    ASStackLayoutSpec *audioSpec = [ASStackLayoutSpec stackLayoutSpecWithDirection:ASStackLayoutDirectionHorizontal spacing:10.0f justifyContent:ASStackLayoutJustifyContentStart alignItems:ASStackLayoutAlignItemsStart children:@[_playButtonNode, layoutSpec]];
...
-(ASLayoutSpec *)defaultLayoutSpecThatFits:(CGSize)maxSize {
    ...
    _scrubberNode.style.preferredSize = CGSizeMake(maxSize.width, 44.0f);
...
-(instancetype)init {
    return [self initWithBackgroundColor:[UIColor mxr_greyBotCell]];
}
